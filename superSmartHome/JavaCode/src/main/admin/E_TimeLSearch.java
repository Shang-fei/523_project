...
public class E_TimeLSearch extends JFrame implements ActionListener{
	...
	public E_TimeLSearch(String manuId,int flag) {
		E_TimeLSearch.this.setVisible(true);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 1000, 550);
		if(flag == 0) {
			this.setTitle("灯光时间搜索系统");
		}
		else {
			this.setTitle("灯光时间搜索系统");
		}
			...
			public void actionPerformed(ActionEvent e) {
				if(textField.getText().equals("") || textField_1.getText().equals("")) {
					...
					try {
				        ...
				        ResultSet rs = null;
				        if(flag == 0) {
				        	rs = stmt.executeQuery("SELECT * FROM light WHERE l_时间 BETWEEN \"" + textField.getText() + "\" AND \"" + textField_1.getText()+ "\"");
				        }
				        else {
				        	rs = stmt.executeQuery("SELECT * FROM light,device WHERE l_时间 BETWEEN \"" + textField.getText() + "\" AND \"" + textField_1.getText()+ "\" AND d_手动号码 = \"" + manuId + "\" AND d_设备号 = l_设备号;");
				        }
